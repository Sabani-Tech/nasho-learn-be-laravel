...
class EloquentKategoriMateriRepositories implements KategoriMateriRepositories
{
    ...
    public function show($uid_kategori_materi, $response)
    {
        if (!$this->validate_exists_kategori_materi_by_id_childs($uid_kategori_materi)) {
            return $response->error_response('kategori materi tidak ditemukan');
        }

        if ($this->validate_exists_kategori_materi_by_id_childs($uid_kategori_materi)) {
            return $response->success_response($this->where_exists_kategori_materi_by_id_childs($uid_kategori_materi), 'Berhasil kategori detail materi');
        }
    }
    ...
    public function store($kategori_materi, $response)
    {
        try {
            ...
            return $response->success_response($kategori_materi, 'Berhasil tambah kategori materi');
        } catch (\Exception $e) {
    ...
    public function update($kategori_materi, $uid_kategori_materi, $response)
    {
        try {
            ...
            if (!$update_kategori_materi) {
                return $response->error_response('id kategori materi salah');
            }

            if ($update_kategori_materi) {
                $this->model->where('id', $uid_kategori_materi)->update($kategori_materi);
                $this->commit_transaction;
                return $response->success_response($kategori_materi, 'Berhasil update kategori materi');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
            return $response->error_response($e->getMessage(), '500', true, $e, \env('APP_ENV'));
    ...
    public function delete($uid_materi, $response)
    {
        try {
            ...
            if (!$delete_kategori_materi) {
                return $response->error_response('kateogri materi sudah di hapus');
            }

            if ($delete_kategori_materi) {
                $this->delete_kategori_materi_by_id_childs($uid_materi);
                $this->commit_transaction;
                return $response->success_response($this->where_exists_kategori_materi_by_id_childs($uid_materi), 'Berhasil delete kategori materi');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
            return $response->error_response($e->getMessage(), '500', true, $e, \env('APP_ENV'));
