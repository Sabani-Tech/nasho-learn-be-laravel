...
//repositories
class EloquentQuisRepositories implements QuisRepositories
{
    private $model,
        $category, $materi,
        $start_transaction,
    ...
    public function show($id_quis, $response)
    {
        if (!$this->validate_exists_quis_by_id_childs($id_quis)) {
            return $response->error_response('quis tidak ditemukan');
        }

        if ($this->validate_exists_quis_by_id_childs($id_quis)) {
            return $response->success_response($this->where_exists_quis_by_id_childs($id_quis), 'Berhasil detail quis');
        }
    }
    ...
    public function store($quis, $response)
    {
        try {
            ...
            return $response->success_response($quis, 'Berhasil tambah quis');
        } catch (\Exception $e) {
    ...
    public function update($quis, $id_quis, $response)
    {
        try {
            ...
            if (!$update_quis) {
                return $response->error_response('id quis salah');
            }
            ...
            if ($update_quis) {
                ...
                $this->commit_transaction;
                return $response->success_response($quis, 'Berhasil update quis');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
    ...
    public function delete($soal_id, $response)
    {
        try {
            ...
            if (!$delete_quis) {
                return $response->error_response('quis sudah di hapus');
            }

            if ($delete_quis) {
                $this->delete_quis_by_id_childs($soal_id);
                $this->commit_transaction;
                return $response->success_response($this->where_exists_quis_by_id_childs($soal_id), 'Berhasil delete quis');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
            return $response->error_response($e->getMessage(), '500', true, \env('APP_ENV'));
