...
class QuisController extends Controller
{
    ...
    public function store(QuisRequest $quisRequest)
    {
        ...
        $materi = $this->quisRepositories->store($quisRequest->safe()->only(['title', 'question', 'point', 'option', 'kategori_materi_id', 'materi_id']));
        return $materi;
    }
    ...
    {
        $materi = $quisRequest->validated();
        $materi = $this->quisRepositories->update($quisRequest->safe()->only(['title', 'question', 'point', 'option', 'kategori_materi_id', 'materi_id']), $id);
        return $materi;
    }
