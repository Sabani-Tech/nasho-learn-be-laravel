...
class EloquentUserManagementRepositories implements UserManagementRepositories
{
    ...
    public function show($id_user, $response)
    {
        if (!$this->validate_exists_user_by_id_childs($id_user)) {
            return $response->error_response('user tidak ditemukan');
        }

        if ($this->validate_exists_user_by_id_childs($id_user)) {
            return $response->success_response($this->where_exists_user_by_id_childs($id_user), 'Berhasil detail user');
        }
    }
    ...
    public function store($user, $response)
    {
        try {
            ...
            return $response->success_response($user, 'Berhasil tambah user');
        } catch (\Exception $e) {
    ...
    public function update($user, $id_user, $response)
    {
        try {
            ...
            if (!$update_user) {
                return $response->error_response('id user salah');
            }

            if ($update_user) {
                $this->model->where('id', $id_user)->update($user);
                $this->commit_transaction;
                return $response->success_response($user, 'Berhasil update user');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
            return $response->error_response($e->getMessage(), '500', true, \env('APP_ENV'));
    ...
    public function delete($id_user, $response)
    {
        try {
            ...
            if (!$delete_user) {
                return $response->error_response('user sudah di hapus');
            }

            if ($delete_user) {
                $this->delete_user_by_id_childs($id_user);
                $this->commit_transaction;
                return $response->success_response($this->where_exists_user_by_id_childs($id_user), 'Berhasil delete user');
            }
        } catch (\Exception $e) {
            $this->rollback_transaction;
            return $response->error_response($e->getMessage(), '500', true, \env('APP_ENV'));
